...
function BookAppointment() {
    ...
    const checkAvailability = async () => {
        try {
            dispatch(showLoading());
            const response = await axios.post('/api/users/check-booking-avilability',
                {
                    doctorId: params.doctorId,
                    date: date,
                    time: time,
                ...
                <div>
                    ...
                    <Row gutter={20} className="mt-5" align="middle">
                        <Col span={8} sm={24} xs={24} lg={8}>
                            <img src="https://img.freepik.com/premium-vector/book-doctor-appointment-card-template_151150-11155.jpg"
                                alt=""
                                width=" 100%"
                                height="400"
                            />
                        ...
                        <Col span={8} sm={24} xs={24} lg={8}>
                            ...
                            <div className="d-flex flex-column pt-2 mt-2">
                                <DatePicker format='DD-MM-YYYY' onChange={(value) => {
                                    setDate(dayjs(value).format().toString())
                                    setIsAvailable(false);
                                }}></DatePicker>
                                <TimePicker format='HH:mm' className="mt-3" onChange={(value) => {


                                    setTime(dayjs(value).toDate());
                                    setIsAvailable(false);
                                ...
                                {!isAvailable && <Button className="primary-button mt-3 full-width" onClick={checkAvailability}>Check Availability</Button>}
                                {isAvailable && (<Button className="primary-button mt-3 full-width" onClick={bookNow}>Book Now</Button>)}
                            </div>
